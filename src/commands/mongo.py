...
def mongo(
    ...
    if clean_only:
        return run_local_cleaner([Mongo(app).config["local_storage_path"]])

    if ftp_clean_only:
        return run_ftp_cleaner([Mongo(app).config["mirror_storage_path"]])

    if rsync_clean_only:
        return run_rsync_cleaner([Mongo(app).config["mirror_storage_path"]])

    return Backupper[TypeConfigContainerMongo].run_backupper(app, ftp_only, ftp, rsync_only, rsync, Mongo(app))
