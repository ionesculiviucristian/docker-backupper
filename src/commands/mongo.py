...
def mongo(
    ...
    if clean_only or ftp_clean_only or rsync_clean_only:
        if clean_only:
            run_local_cleaner([Mongo(app).config["local_storage_path"]])
        if ftp_clean_only:
            run_ftp_cleaner([Mongo(app).config["mirror_storage_path"]])
        if rsync_clean_only:
            run_rsync_cleaner([Mongo(app).config["mirror_storage_path"]])
        return
