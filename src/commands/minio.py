...
def minio(
    ...
    if clean_only:
        return run_local_cleaner([MinIO(app).config["local_storage_path"]])

    if ftp_clean_only:
        return run_ftp_cleaner([MinIO(app).config["mirror_storage_path"]])

    if rsync_clean_only:
        return run_rsync_cleaner([MinIO(app).config["mirror_storage_path"]])

    return Backupper[TypeConfigContainerMinio].run_backupper(app, ftp_only, ftp, rsync_only, rsync, MinIO(app))
