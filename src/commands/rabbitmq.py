...
def rabbitmq(
    ...
    if clean_only:
        return run_local_cleaner([RabbitMQ(app).config["local_storage_path"]])

    if ftp_clean_only:
        return run_ftp_cleaner([RabbitMQ(app).config["mirror_storage_path"]])

    if rsync_clean_only:
        return run_rsync_cleaner([RabbitMQ(app).config["mirror_storage_path"]])

    return Backupper[TypeConfigContainerRabbitmq].run_backupper(app, ftp_only, ftp, rsync_only, rsync, RabbitMQ(app))
